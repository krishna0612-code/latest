...
const BaseLogin = ({
  ...
  const userTypeContent = {
    user: {
      title: "Patient Portal",
      quote:
        "Your health is our priority. Access your medical records, book appointments, and manage prescriptions all in one place.",
      gradient: `linear-gradient(135deg, ${colors.primary} 0%, ${colors.darktext} 100%)`,
      features: [
        { icon: "📋", text: "View medical records & history" },
        { icon: "📅", text: "Book doctor appointments online" },
    ...
    vendor: {
      title: "Pharmacy Vendor Portal",
      quote:
        "Manage your pharmacy inventory, process orders, and serve patients efficiently with our comprehensive vendor tools.",
      gradient: `linear-gradient(135deg, ${colors.mint} 0%, #00695C 100%)`,
      features: [
        { icon: "📦", text: "Manage medicine inventory" },
        { icon: "📋", text: "Process patient orders" },
    ...
    doctor: {
      title: "Doctor's Portal",
      quote:
        "Provide exceptional care to your patients with our comprehensive medical practice management tools.",
      gradient: `linear-gradient(135deg, ${colors.primary} 0%, #00695C 100%)`,
      features: [
        { icon: "👨‍⚕️", text: "Manage patient appointments" },
        { icon: "📄", text: "Write digital prescriptions" },
    ...
    guardian: {
      title: "Guardian Portal",
      quote:
        "Care for your loved ones. Monitor their health, manage appointments, and ensure they receive the best care.",
      gradient: `linear-gradient(135deg, ${colors.mint} 0%, ${colors.primary} 100%)`,
      features: [
        { icon: "👨‍👩‍👧", text: "Monitor dependent health records" },
        { icon: "📅", text: "Schedule appointments for dependents" },
    ...
    wife: {
      title: "Family Care Portal",
      quote:
        "Manage your family's healthcare needs from one place. Stay informed and proactive about family health.",
      gradient: `linear-gradient(135deg, #E91E63 0%, ${colors.primary} 100%)`,
      features: [
        { icon: "👨‍👩‍👧‍👦", text: "Manage family health profiles" },
        { icon: "📋", text: "Access family medical records" },
  ...
  const shouldShowOTP = () => {
    return (
      userType === "user" || userType === "guardian" || userType === "wife"
    );
  };
  ...
  const handleEmailLogin = async (e) => {
    ...
    try {
      ...
      showToastMessage(
        `Welcome back, ${data.user?.fullName || data.user?.email || email}!`,
        "success"
      );
  ...
  const handleVerifyOTP = async (e) => {
    ...
    try {
      ...
      const res = await fetch("http://127.0.0.1:8000/api/login/verifyotp/", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ phone: phoneNumber, otp, userType }),
      });
      ...
      showToastMessage(
        `Welcome back, ${data.user?.fullName || userDetails.label}!`,
        "success"
      );
    ...
    <div className="login-container">
      ...
      <div className="main-card">
        ...
        >
          ...
          <h2 className="user-title">{currentContent.title}</h2>

          <p className="user-quote">{currentContent.quote}</p>
        ...
        <div className="right-section">
          <div className="form-header">
            ...
            <p className="login-subtitle">
              {isLinkedAccount
                ? `Linked access to ${
                    userType === "guardian" ? "dependent" : "family"
                  } account`
                : shouldShowOTP()
          ...
          {signupSuccess && (
            <div className="success-message">
              ...
              <div>
                <h4>Account Created Successfully!</h4>
                <p>
                  Your account has been created. Please login with your
                  credentials.
                </p>
              </div>
            </div>
          ...
          {shouldShowOTP() && (
            <div className="login-method-toggle">
              <button
                ...
                style={{
                  backgroundColor:
                    loginMethod === "email" ? colors.primary : "",
                  borderColor: loginMethod === "email" ? colors.primary : "",
                }}
              >
                <svg
          ...
          {!shouldShowOTP() || loginMethod === "email" ? (
            <form onSubmit={handleEmailLogin}>
              <div className="input-group">
                <label>
                  {userType === "delivery" ? "Partner Email" : "Email Address"}
                </label>
                <input
                  type="email"
                  value={email}
                  ...
                  className={formErrors.email ? "error" : ""}
                  style={{
                    borderColor: formErrors.email ? "#f44336" : colors.softbg,
                  }}
                />
                {formErrors.email && (
                  <div className="error-message">{formErrors.email}</div>
            ...
            <form onSubmit={showOtpField ? handleVerifyOTP : handleSendOTP}>
              <div className="input-group">
                ...
                <div className="phone-input-wrapper">
                  ...
                  <input
                    ...
                    className={formErrors.phone ? "error" : ""}
                    style={{
                      borderColor: formErrors.phone ? "#f44336" : colors.softbg,
                    }}
                    maxLength={10}
                  />
                </div>
              ...
              {showOtpField && (
                <div className="input-group">
                  ...
                  <div className="otp-hint">OTP sent to +91{phone}</div>
                </div>
              )}
          ...
          {shouldShowOTP() && loginMethod === "otp" && !showOtpField && (
            <div className="demo-note">
              <p>
                💡 <strong>Demo Note:</strong> Enter any valid Indian mobile
                number (10 digits). OTP is always <strong>1234</strong> for
                testing.
              </p>
            </div>
          )}
          ...
          {shouldShowOTP() && loginMethod === "otp" && showOtpField && (
            <div className="demo-note">
              <p>
                🔑 <strong>Demo OTP:</strong> Use <strong>1234</strong> to login
