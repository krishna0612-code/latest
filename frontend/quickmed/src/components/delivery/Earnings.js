...
const Earnings = ({ deliveryData }) => {
  ...
  const getFilteredMetrics = () => {
    const selectedMonth = initialMonthsData.find(
      (month) => month.value === earningFilter
    );
    ...
    switch (earningFilter) {
      case "today":
        return {
          ...
          averagePerDelivery:
            deliveryData.stats.completed > 0
              ? Math.round(
                  deliveryData.stats.todayEarnings /
                    deliveryData.stats.completed
                )
              : 0,
          efficiency: "94%",
          activeHours: "8h 30m",
      ...
      case "month":
        ...
        return {
          ...
          averagePerDelivery: Math.round(
            currentMonthData.earnings / currentMonthData.deliveries
          ),
          efficiency: "93%",
      ...
      default:
        ...
        if (selectedMonth) {
          return {
            ...
            averagePerDelivery: Math.round(
              selectedMonth.earnings / selectedMonth.deliveries
            ),
            efficiency:
    ...
    <div style={styles.mainContent}>
      ...
      <div style={styles.earningsSummary}>
        <div style={styles.earningStats}>
          ...
          <div style={styles.earningStat}>
            <h3 style={styles.earningAmount}>{metrics.cancelledDeliveries}</h3>
            <p style={styles.earningLabel}>Cancelled Orders</p>
            <div style={styles.metricDetail}>
              <span style={styles.metricValue}>
                {metrics.totalDeliveries > 0
                  ? Math.round(
                      (metrics.cancelledDeliveries / metrics.totalDeliveries) *
                        100
                    )
                  : 0}
                %
      ...
      {selectedMonth && (
        <div style={styles.earningsHistory}>
          <h3 style={styles.sectionTitle}>
            Month Details - {selectedMonth.label}
          </h3>
          <div style={styles.earningStats}>
            <div style={styles.earningStat}>
              <h3 style={styles.earningAmount}>
          ...
          <>
            <div style={styles.earningsList}>
              {currentMonthsData.length > 0 ? (
                  ...
                  >
                    <div style={styles.earningDate}>
                      ...
                      <div style={styles.earningMeta}>
                        ...
                        <span style={styles.metaBadge}>
                          {month.value.includes("january")
                            ? "93%"
                            : month.value.includes("december")
                            ? "90%"
                            : month.year === 2023
                            ? "85%"
                            : "88%"}{" "}
                ...
                <div style={styles.noData}>No earnings data available</div>
              )}
            </div>
