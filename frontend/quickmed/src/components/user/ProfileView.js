...
const ProfileView = ({ setActiveView }) => {
  ...
  const { profile, updateProfile, updateProfilePhoto, removeProfilePhoto } =
    useProfile();
  ...
  const validateLocalForm = useCallback(() => {
    ...
    if (
      ...
      !/^[A-Za-z\s]{0,}$/.test(localProfile.lastName)
    ) {
    ...
    if (!localProfile.city.trim()) {
      ...
      errors.city =
        "City should contain only letters and be at least 2 characters";
    }
    ...
    if (!localProfile.state.trim()) {
      ...
      errors.state =
        "State should contain only letters and be at least 2 characters";
    }
  ...
  const getInputStyle = (fieldName) => {
    ...
    const errorStyle =
      localIsFormTouched && localFormErrors[fieldName]
        ? {
  ...
  const isProfileComplete = () => {
    ...
    return requiredFields.every(
      (field) => localProfile[field] && localProfile[field].trim()
    );
  };
  ...
  return (
    <>
      ...
      <div style={styles.profileContainer}>
        ...
        {/* Save Status Display */}
        {saveStatus && <div style={getSaveStatusStyle()}>{saveStatus}</div>}
